...
export default function CategoryManager({ auth }) {
  ...
  const renderTree = (nodes) => (
    <ul className="space-y-2">
      {nodes.map(cat => (
        <CategoryNode key={cat.id} cat={cat} depth={0} categories={categories} auth={auth} load={load} remove={remove} setMessage={setMessage} />
      ))}
    </ul>
  );
...
function CategoryNode({ cat, depth, categories, auth, load, remove, setMessage }) {
  ...
  return (
    <li key={cat.id}>
      ...
      {isEditing && (
        <EditModal title={`Modifier « ${cat.name} »`} onClose={() => setIsEditing(false)}>
      ...
      {cat.children?.length > 0 && (
        <ul className={`${depth > 0 ? 'ml-6 border-l border-lgo-border/50 pl-3' : ''} mt-2 space-y-2`}>
          {cat.children.map(child => (
            <CategoryNode key={child.id} cat={child} depth={depth + 1} categories={categories} auth={auth} load={load} remove={remove} setMessage={setMessage} />
          ))}
        </ul>
      )}
    </li>
