  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        {pages.map(page => (
          <PageRow key={page.id} page={page} auth={auth} load={load} remove={remove} setMessage={setMessage} />
        ))}
      </div>
    </div>
  );
...
function PageRow({ page, auth, load, remove, setMessage }) {
  ...
  return (
    <div className="border border-lgo-border rounded-lg p-3 bg-lgo-card/50 space-y-3">
      ...
      {isEditing && (
        <EditModal title={`Modifier « ${page.title} »`} onClose={() => setIsEditing(false)}>
